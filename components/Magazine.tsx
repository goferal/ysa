...
export function MagazineCover({ guide, index = 0, className = '' }: { guide: Guide; index?: number; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <p className="absolute left-[12%] top-[7%] text-[0.55em] uppercase tracking-[0.22em] text-ink/60">Style guide</p>
...
/**
 * Guides fanned out like magazines on a table. Up to four; the rest
 * are implied by the count. Each cover is a link.
 */
export function MagazineStack({ guides }: { guides: Guide[] }) {
  ...
  return (
    <div className="relative mx-auto aspect-[4/3] w-full max-w-md text-[14px] sm:text-[16px]">
      ...
      {shown.length === 1 && (
        <p className="absolute left-[48%] top-[30%] max-w-[46%] rotate-3 font-hand text-2xl leading-tight text-ink/80">
          first of many; more types on the way
        </p>
      )}
    </div>
  );
