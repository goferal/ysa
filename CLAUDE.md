...
- Fonts: Fraunces for display (a soft modern serif, weight 500, wonky alternates off; big headlines sit in a narrow measure (`max-w-2xl`/`3xl`, so the hero stacks into three lines) with line-height about 1.08, never full width; Bodoni Moda was too hairline to read live and Abril Fatface too much of a poster), Karla for body and kickers, Caveat for asides. Display headings are near-black (`noir`, matching the logo); body copy is maroon.
- Motion is CSS only, no libraries: blocks lift in on first scroll-in (`components/Reveal.tsx` hides a block only after mount and only if it is below the fold, so crawlers and the first screen never wait), the hero words rise out of a mask on load, and the three lens cards stack (`position: sticky`) as you scroll. Everything respects `prefers-reduced-motion`. Reference for the feel MC likes: eliza.com.
