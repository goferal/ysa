...
- Motion is CSS only, no libraries: blocks lift in on first scroll-in (`components/Reveal.tsx`, hidden state gated on `html.js` so crawlers see everything), the hero words rise out of a mask on load, and the three lens cards stack (`position: sticky`) as you scroll. Everything respects `prefers-reduced-motion`. Reference for the feel MC likes: eliza.com.
