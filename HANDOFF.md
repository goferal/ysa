...
- **Sep 3, after MC saw the live site ("boring, not unique, too much text"):** homepage redesigned as an editorial magazine spread (masthead nav, numbered kickers, hairline rules, white "sheet" pages, a maroon band, a ticker) in Abril Fatface on a paper-white ground (the blush washes were "window dressing"), copy cut to one line per section, plus a four-question archetype quiz (`components/Quiz.tsx`). The "no cards" rule is relaxed to sheets on the homepage and white cards on inner pages. MC's reference for layout, motion, and vibe is eliza.com (Webflow + GSAP: off-white ground, one dark section, soft serif at modest sizes, uppercase taglines, line-mask reveals, sticky stacking cards); the site now uses Fraunces, near-black headings (logo color), CSS reveals, and stacking lens cards to match it without a library.
